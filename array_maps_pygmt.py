...
def pygmt_array_earthquakes(array_lats, array_lons, array_names, 
    ...
    if run_topo == True:
    # Load topography
        load_grid = pygmt.datasets.load_earth_relief(resolution= '30s',#'30s',#'10m', 
                                                     region=region, 
                                                     registration=None, 
                                                     data_source='igpp', 
                                                     use_srtm=False) #30s
...
def pygmt_baz_error(array_lat, array_lon, array_name, earthquake_lats, 
    ...
    #pygmt.makecpt(cmap=CPT_Option)  #, series=[-1.5, 0.3, 0.01])
        ...
        fig.plot(x=pos_error['lon'],
                y=pos_error['lat'],
                direction = [pos_error['vec_direction'],0.06*pos_error['baz_error']],#0.045*pos_error['baz_error']],
        ...
        fig.plot(x=neg_error['lon'],
                y=neg_error['lat'],
                direction = [neg_error['vec_direction']-180, 0.06*neg_error['baz_error']],#0.045*neg_error['baz_error']],
                #direction = [baz_array_pygmt, [length]],
                style="v0.5c+ea",
                #fill = "cyan4",
                fill="royalblue",
...
def pygmt_slow_error(array_lat, array_lon, array_name, earthquake_lats, 
    ...
    extend_rect = 3 #5 (regional), 0.5 (local)
    region_rect = str(left-extend_rect)+"/"+str(bottom-extend_rect)+"/"+str(right+extend_rect)+"/"+str(top+extend_rect)+"r"
