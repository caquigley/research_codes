...
def read_params(params):
    ...
    time_analysis_type = params["trigger"]["time_analysis_type"]
    ...
    return (net, sta, loc, chan, client_str, min_stations, remove_stations, 
            ...
            plot_histogram, save_fig, fig_path, drop_pow, pow_thresh, time_analysis_type)
...
if __name__ == "__main__":
    ...
    (net, sta, loc, chan, client_str, min_stations, remove_stations, 
            ...
            baz_error_plot, slow_error_plot, baz_error_map, slow_error_map,
            plot_histogram, 
            save_fig, fig_path, drop_pow, pow_thresh, time_analysis_type) = read_params(params)
    ...
    if analysis_type == 'time_series':
        ...
        if time_analysis_type == 'baz':
            ...
            var = slow_error_list
        baz_stacked = np.column_stack(var) #baz_error_list
        q05, q95 = np.nanquantile(baz_stacked, [0.05, 0.95], axis=1)
        quantiles = np.abs(q05) + np.abs(q95)  
        
        time_stacked = np.column_stack(tlist)
        time_bins = np.mean(time_stacked, axis = 1)

        #Plot quantiles
        quantiles_time_series(time_bins, quantiles, -4, 8, time_analysis_type= time_analysis_type, save = False)
        quant_df = pd.DataFrame({
            ...
            'q95': q95})
        #print(quant_df)
        #quant_df.to_csv('/Users/cadequigley/Downloads/Research/unalaska_arrays_paper/'+array_name+'_'+processing+'_'+time_analysis_type+'_time_quantiles.csv')
        
        #Density Plot------------------
        time_series = np.concatenate(tlist)
        baz_error = np.concatenate(baz_error_list)
        slow_error = np.concatenate(slow_error_list)
        
        mask = np.isfinite(baz_error)

        time_series = time_series[mask]
        baz_error = baz_error[mask]
        slow_error = slow_error[mask]
        
        if time_analysis_type == 'baz':
            vals = baz_error
        if time_analysis_type == 'slow':
            vals = slow_error
            

        #print(baz_error)
        time_series_density(time_series, vals, time_series_start, 
                            time_series_end, time_bins, q05, q95, 
                            time_analysis_type=time_analysis_type, 
                            save = False, 
                            path = '/Users/cadequigley/Downloads/Research/unalaska_arrays_paper/figure_components/'+array_name+'_'+processing+'_'+time_analysis_type+'_time_density_plot.png')
    ...
    if baz_error_plot == True:
        baz_error_spatial(df["backazimuth"], df["baz_error"], model_data,
            ...
            path=fig_path + "baz_error_spatial_power_supp.png")   

    if slow_error_plot == True:
        slow_error_spatial(df["backazimuth"], df["slow_error"], model_data,
            color_data, color_label, niazi=False, save=save_fig,
            path=fig_path + "slow_error_spatial_power_supp.png")  
