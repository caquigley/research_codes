...
df = pd.DataFrame(df[df['magnitude']>= 6]).reset_index() #3 for noise characteristics, 6 for rest

#%%
# Pull out date for each earthquake
dates_of_events = [df['time_utc'][i][:10] for i in range(len(df))]

# Find frequency of events on each day
# (renamed from `time` -> `time_series` to avoid shadowing the `time` module,
#  which was breaking time.sleep() inside fetch_psd_for_station)
time_series = pd.Series(dates_of_events)
    ...
    for start_time in filtered_dates:
        ...
        for attempt in range(3):
            try:
                ...
                if e.code == 404:
                    # No PSD available for this station/day - not a transient
                    # error, so don't waste retries on it.
                    print(f'No data for {station} on {start_time} (404), skipping.')
                    break
                else:
                    wait = 5 * (attempt + 1)
                    if attempt < 2:
                        print(f'HTTP {e.code} for {station} on {start_time}: {e}. Retrying in {wait}s...')
                        time.sleep(wait)
                    else:
...
full_df.to_csv(figpath+array+'_psds_all_dates.csv')
#full_df.to_csv(figpath+array+'_psds.csv')